...
const AGENT_META: Record<string, {
  ...
  axon: {
    label:       'AXON',
    description: 'Analyste stratégique — contexte vitrine et pipeline',
    color:       'text-[#39FF14]',
    accent:      '#39FF14'
  },
  business: {
    label:       'BUSINESS',
    description: 'Assistant business — mentor clients inscrits',
    color:       'text-blue-400',
    accent:      '#60a5fa'
  },
  commercial: {
    label:       'COMMERCIAL',
    description: 'Agent vocal commercial — ElevenLabs',
  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        const found = (json.data ?? []).find((a: AgentConfig) => a.agent_id === agentId)
        setAgent(found ?? null)
      } catch (err: any) {
      ...
      <div className="flex flex-col h-screen bg-[#0A0A0A] font-mono overflow-hidden">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-white/[0.06] bg-[#0A0A0A] shrink-0">
          <div className="flex items-center gap-4">
            ...
            <div className="flex items-center gap-3">
              <div className="flex flex-col">
                <span className={`text-[14px] font-bold tracking-tight ${meta.color}`}>
                  {meta.label}
                </span>
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              onClick={() => router.push(`/admin/hq/agents/${agentId}/settings`)}
              className="flex items-center gap-2 px-3 py-1.5 rounded-lg border border-white/10 text-white/30 text-[11px] hover:text-white hover:border-white/20 transition-all"
            >
          ...
          {messages.length === 0 && !loadingAgent && (
            <div className="flex flex-col items-center justify-center h-full space-y-8 max-w-[600px] mx-auto">
              <div className="space-y-2 text-center">
                <h2 className={`text-[22px] font-bold tracking-tight ${meta.color}`}>
                  {meta.label}
                </h2>
              ...
              {msg.role === 'assistant' && (
                <div className="flex gap-3 max-w-[75%]">
                  ...
                  <div className="space-y-1">
                    <span className="text-[10px] font-medium text-white/25 ml-1">
                      {meta.label}
                    </span>
              ...
              {msg.role === 'user' && (
                <div className="flex gap-3 max-w-[75%]">
                  <div className="space-y-1 items-end flex flex-col">
                    <span className="text-[10px] font-medium text-white/25 mr-1">
                      Amadou
                    </span>
        ...
        {/* ── INPUT ────────────────────────────────────────────────────── */}
        <div className="shrink-0 px-6 py-4 border-t border-white/[0.06] bg-[#0A0A0A]">
